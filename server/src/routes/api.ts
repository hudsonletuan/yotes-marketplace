...
const sendOTP = (email: string, otp: string, type: string) => {
    ...
    const mailOptions = {
        ...
        html: `
        <h3>Your Yotes Marketplace OTP is ${otp}.</h3>
        <p>This code is only valid for 5 minutes!</p>
        <p>Do not share this code with anyone!</p>
        <p>Thank you for using Yotes Marketplace!</p>
        <p>Any questions or concerns, please contact: tuan.le@yotes.collegeofidaho.edu</p>
        `
        // html: `
        // <!DOCTYPE html>
        // <html lang="en">
...
router.post('/newpost', upload.array('media'), async (req, res) => {
    try {
        const { userId, username, userImg, topic, caption, price, status, location } = req.body;
...
router.put('/editpost/:id', upload.array('media'), async (req, res) => {
    try {
        const postId = req.params.id;
        const { userId, username, topic, caption, price, status, location, existingMediaUrls, removedMediaUrls } = req.body;
        ...
        const updatedPost = await Post.findByIdAndUpdate(postId, { userId, username, $push: { versions: { userId, username, topic, caption, price, status, location, uploaded: [...existingMediaUrlsMroutered, ...newMedia] } } }, { new: true });
        if (!updatedPost) {
            return res.status(404).json({ message: 'Post not found' });
        } else {
            await Promise.all(deletePromises);
